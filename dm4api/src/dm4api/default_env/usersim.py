...
class UserSim:
    ...
    def initConstraints(self):
        ...
        constraints['out_of_results_known'] = False
    ...
    def createUserAction(self, system_action):
        ...
        if (((system_action["action"] in ["sugg-all", "change-page"] and (not system_action["list"] or len(system_action['list']) == 0)) or 
            (system_action["action"] in ["sugg", "sugg-info-all"] and (not system_action["function"] or len(system_action["function"]) == 0)))):
                self.constraints["out_of_results_known"] = True
        ...
        if (self.constraints["out_of_results_known"] and self.constraints["incorrect_keywords"]):
            ...
            while "action" not in user_action:
                if actions[0] == "info":
                    if system_action["action"] == "eli-kw":
                        ...
                        # Trying to answer kw question but can't
                            ...
                            user_action["query"] = self.constraints['target_query']
                            # print("UA08")
                    elif (system_action["action"] in ["sugg-all", "change-page"] 
                                and self.constraints["target_function_named"] not in system_action["list"]
                                and len(system_action["list"]) > 0):
                ...
                elif (actions[0] == "eli-sugg" and not self.constraints["out_of_results_known"]):
