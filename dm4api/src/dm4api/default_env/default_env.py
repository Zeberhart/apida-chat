...
class DefaultEnv(Env):
    ...
    def user_step(self):
        """
            Runs one "User step" -- passes the current env state to the user to generate a response
            and update the env state.
        # Returns
            observation (object): The initial observation of the space. Initial reward is assumed to be 0.
        """
        user_action = self.user.respond(self.history["system_action"])
    ...
    def interactive_user_step(self, user_action):
        """
            Uses a given user_action to update the env, and produce an observation
        # Returns
            observation (object): The initial observation of the space. Initial reward is assumed to be 0.
        """
        self.processUserAction(user_action)
    ...
    def interactive_system_step(self, system_action):
        """
            Uses a given system_action to update the env,
        # Returns
            parsed_system_action (object): The system action in a human-readable format
        """
        self.current_turn += 1
        parsed_system_action = self.parseAction(system_action)
        self.processSystemAction(parsed_system_action)

        # Used for logging and evaluation
        self.updateMetaState(parsed_system_action)

        return parsed_system_action
